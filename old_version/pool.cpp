...
Pool::Pool(
	std::function<bool(ImgData &)> drawOnRawImageCallback,
	std::function<bool(ImgData &)> getRenderedImageCallback) :
	m_drawOnRawImageCallback(drawOnRawImageCallback),
	m_getRenderedImageCallback(getRenderedImageCallback)
{
}
...
bool Pool::DrawOnRawImage(void) {
	...
	if (m_inactives.size() > 0) {
		...
		image.isReady = false;
		m_drawOnRawImageCallback(image);
		image.isReady = true;
...
bool Pool::GetRenderedImage(void) {
	if (m_ready == false)
		return false;
	...
	if (m_actives.size() > 0 && m_actives.front().isReady == true) {
		...
		image.isReady = false;
		m_getRenderedImageCallback(image);
		image.isReady = true;
